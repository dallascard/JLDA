...
public class GaussianLDAmentions {
    ...
    public static void main(String args[]) throws Exception {
        ...
        GaussianLDAmentionsSampler sampler = new GaussianLDAmentionsSampler(input_dir, vec_size);
...
class GaussianLDAmentionsSampler {
    ...
    private int tuple_mention[];
    private int mention_entity[];
    private HashMap<Integer, List<Integer>> mention_tuples;
    private HashMap<Integer, List<Integer>> entity_mentions;
    ...
    public GaussianLDAmentionsSampler(String input_dir, int dx) throws Exception {

        Data.D = dx;

        Path tuple_vocab_file = Paths.get(input_dir, "tuple_vocab.json");
        Path tuple_entity_file = Paths.get(input_dir, "tuple_entity.json");
        Path mention_entity_file = Paths.get(input_dir, "mention_entity.json");
        ...
        Path tuple_mention_file = Paths.get(input_dir, "tuple_mention.json");
        Path vectors_file = Paths.get(input_dir, "mention_vectors.json");
        ...
        JSONArray mention_entity_json = (JSONArray) parser.parse(new FileReader(mention_entity_file.toString()));
        ...
        JSONArray tuple_mention_json = (JSONArray) parser.parse(new FileReader(tuple_mention_file.toString()));
        JSONArray mention_vectors_json = (JSONArray) parser.parse(new FileReader(vectors_file.toString()));
        ...
        n_mentions = mention_entity_json.size();
        System.out.println("n_mentions=" + n_mentions);
        ...
        assert n_mentions == mention_vectors_json.size();
        ...
        for (int i = 0; i < n_mentions; i++) {
            mention_entity[i] = ((Long) mention_entity_json.get(i)).intValue();
            JSONArray vector = (JSONArray) mention_vectors_json.get(i);
            ...
            if (entity_mentions.get(mention_entity[i]) == null) {
                List<Integer> mentions = new ArrayList<>();
                mentions.add(i);
                entity_mentions.put(mention_entity[i], mentions);
            }
            // otherwise, add this tuple to the appropriate list
            else {
                // probably a better way to do this...
                List<Integer> mentions = entity_mentions.get(mention_entity[i]);
                mentions.add(i);
                entity_mentions.put(mention_entity[i], mentions);
            }
        }
        ...
        for (int i = 0; i < n_tuples; i++) {
            ...
            tuple_mention[i] = ((Long) tuple_mention_json.get(i)).intValue();
            ...
            // if we haven't seen this entity before, make a new list for it
            if (mention_tuples.get(tuple_mention[i]) == null) {
                List<Integer> tuples = new ArrayList<>();
                tuples.add(i);
                mention_tuples.put(tuple_mention[i], tuples);
            }
            // otherwise, add this tuple to the appropriate list
            else {
                // probably a better way to do this...
                List<Integer> tuples = mention_tuples.get(tuple_mention[i]);
                tuples.add(i);
                mention_tuples.put(tuple_mention[i], tuples);
            }
        }
        ...
        data_vectors = new DenseMatrix64F[n_mentions]; //splitting into vectors
    ...
    void initialize(int n_personas, int n_topics) {
        ...
        */
        ...
        // randomly assign mentions to topics
        for (int j = 0; j < n_mentions; j++) {
            ...
            DenseMatrix64F mention_vector_transpose = new DenseMatrix64F(1, Data.D);
            DenseMatrix64F square_vector = new DenseMatrix64F(Data.D, Data.D);
            mention_vector_transpose = CommonOps.transpose(data_vectors[j], mention_vector_transpose);
            //Multiply x_ix_i^T and add it to the sum_squared for this topic
            CommonOps.mult(data_vectors[j], mention_vector_transpose, square_vector);
            CommonOps.add(sum_squared, square_vector, sum_squared);
        }

        // keep track of tuple assignements for now, for later interpretability
        ...
        */
        ...
        System.out.println("number of entities=" + n_entities);
        System.out.println("number of mentions=" + n_mentions);
    ...
    public int[][][] run(double alpha, double beta, int n_iter, int burn_in, int subsampling, String outputDir, double slice_width) throws Exception {
        ...
        List<Integer> mention_order = new ArrayList<>();
        for (int i = 0; i < n_mentions; i++) {
            mention_order.add(i);
        }
        ...
        for (int i=0; i < n_iter; i++) {
            ...
            for (int q=0; q < n_entities; q++) {
                ...
                List<Integer> mentions = entity_mentions.get(e);
                assert mentions != null;
                for (int t : mentions) {
                    int topic_t = mention_topics[t];
                ...
                if (p_e != p) {
                    for (int m : mentions) {
                        int topic_m = mention_topics[m];
                        // transfer the persona topic counts to the new persona
                        persona_topic_counts[p_e][topic_m] -= 1;
                        persona_counts[p_e] -= 1;
            ...
            // then sample topics
            for (int q=0; q < n_mentions; q++) {
                int j = mention_order.get(q);
                int e_j = mention_entity[j];
                int z_j = mention_topics[j];
                int p_j = entity_personas[e_j];
                ...
                for(int k = 0; k < n_topics; k++)
                {
                    ...
                    double count = persona_topic_counts[p_j][k] + beta;
            ...
            if (i > burn_in) {
                if (i % subsampling == 0) {
                    ...
                    for (int j = 0; j < n_tuples; j++) {
                        ...
                        int z_j = mention_topics[m_j];
                        int p_j = entity_personas[e_j];
                        t_topic_vocab_counts[z_j][v_j] += 1;
                        t_persona_role_vocab_counts[p_j][r_j][v_j] += 1;
                    }
                    for (int j = 0; j < n_head_words; j++) {
                        int e_j = head_entity_list[j];
                        int v_j = head_vocab_list[j];
                        int p_j = entity_personas[e_j];
                        t_persona_head_word_counts[p_j][v_j] += 1;
                    }
        ...
        for (int p=0; p < 1; p++) {
            for (int r=0; r < 1; r++) {
                for (int k = 0; k < n_topics; k++) {
                    ...
                    log_p += Gamma.logGamma(beta + persona_topic_counts[p][k]) - Gamma.logGamma(beta);
                }
                //log_p -= Gamma.logGamma(n_topics * beta + persona_role_counts[p][r]) + Gamma.logGamma(n_topics * beta);
                log_p -= Gamma.logGamma(n_topics * beta + persona_counts[p]) + Gamma.logGamma(n_topics * beta);
            }
        }
        return log_p;
    }
    ...
    private double log_multivariate_t_density(DenseMatrix64F x, int k)
    {
        ...
        int count = topic_counts[k]; //this is the prior
